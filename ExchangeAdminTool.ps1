...
function New-LabeledComboBox {
    ...
    $cmb.Location = [System.Drawing.Point]::new([int]$X, [int]($Y + 20))
    $cmb.Size = [System.Drawing.Size]::new([int]$Width, 24)
...
function ConvertTo-DataTable {
    ...
    foreach ($item in $InputObject) {
        ...
        foreach ($name in $properties) {
            $value = $item.$name
            if ($null -eq $value) { $row[$name] = '' }
            else { $row[$name] = [string]$value }
        }
...
$tabCurrent.Text = 'Current Permissions'
$tabCurrent.BackColor = [System.Drawing.Color]::White
$tabs.TabPages.Add($tabCurrent)

$txtViewIdentity = New-LabeledTextBox -Parent $tabCurrent -Label 'Mailbox / Group Identity' -X 20 -Y 20 -Width 320
$btnViewMailboxPerms = New-ThemeButton -Parent $tabCurrent -Text 'View Mailbox Permissions' -X 360 -Y 40 -Width 190
$btnViewCalendarPerms = New-ThemeButton -Parent $tabCurrent -Text 'View Calendar Permissions' -X 560 -Y 40 -Width 190 -BackColor $Theme.PrimaryDark
$btnViewGroupMembers = New-ThemeButton -Parent $tabCurrent -Text 'View Group Members' -X 760 -Y 40 -Width 170 -BackColor $Theme.AccentDark
$btnViewSharedPerms = New-ThemeButton -Parent $tabCurrent -Text 'View Shared Mailbox Access' -X 360 -Y 82 -Width 230
$btnViewOOF = New-ThemeButton -Parent $tabCurrent -Text 'View Out of Office' -X 600 -Y 82 -Width 170 -BackColor $Theme.AccentDark

$gridCurrent = New-Object System.Windows.Forms.DataGridView
$gridCurrent.Location = [System.Drawing.Point]::new(20, 140)
$gridCurrent.Size = [System.Drawing.Size]::new(980, 380)
$gridCurrent.ReadOnly = $true
$gridCurrent.AllowUserToAddRows = $false
$gridCurrent.AllowUserToDeleteRows = $false
$gridCurrent.AutoSizeColumnsMode = 'Fill'
$tabCurrent.Controls.Add($gridCurrent)

function Refresh-DistributionGroups {
    Assert-Connected
    $groups = Get-DistributionGroup -ResultSize Unlimited | Sort-Object DisplayName
    $values = @($groups | ForEach-Object { $_.PrimarySmtpAddress.ToString() })
    Set-ComboValues -Combo $cmbGroupName -Values $values
}
...
$btnViewMailboxPerms.Add_Click({
    global:InvokeSafely -Script {
        Assert-Connected
        $identity = $txtViewIdentity.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($identity)) { throw 'Enter a mailbox or group identity first.' }
        $data = Get-MailboxPermission -Identity $identity | Select-Object User,AccessRights,IsInherited,Deny
        Set-ResultGrid -Grid $gridCurrent -Data $data
    } -SuccessMessage 'Mailbox permissions loaded.'
})

$btnViewCalendarPerms.Add_Click({
    global:InvokeSafely -Script {
        Assert-Connected
        $identity = $txtViewIdentity.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($identity)) { throw 'Enter a mailbox or group identity first.' }
        $data = Get-MailboxFolderPermission -Identity "$identity`:\Calendar" | Select-Object User,AccessRights,SharingPermissionFlags
        Set-ResultGrid -Grid $gridCurrent -Data $data
    } -SuccessMessage 'Calendar permissions loaded.'
})
...
$btnViewGroupMembers.Add_Click({
    global:InvokeSafely -Script {
        ...
        $identity = $txtViewIdentity.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($identity)) { throw 'Enter a mailbox or group identity first.' }
        $data = Get-DistributionGroupMember -Identity $identity -ResultSize Unlimited | Select-Object Name,PrimarySmtpAddress,RecipientType
        Set-ResultGrid -Grid $gridCurrent -Data $data
...
$btnViewSharedPerms.Add_Click({
    global:InvokeSafely -Script {
        ...
        $identity = $txtViewIdentity.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($identity)) { throw 'Enter a mailbox or group identity first.' }
        $data = Get-MailboxPermission -Identity $identity | Where-Object { -not $_.IsInherited -and $_.User -ne 'NT AUTHORITY\SELF' } | Select-Object User,AccessRights,IsInherited
        Set-ResultGrid -Grid $gridCurrent -Data $data
    } -SuccessMessage 'Shared mailbox access loaded.'
})
...
$btnViewOOF.Add_Click({
    global:InvokeSafely -Script {
        ...
        $identity = $txtViewIdentity.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($identity)) { throw 'Enter a mailbox or group identity first.' }
        $cfg = Get-MailboxAutoReplyConfiguration -Identity $identity
        $data = @([pscustomobject]@{
        ...
        Set-ResultGrid -Grid $gridCurrent -Data $data
    } -SuccessMessage 'Out of Office configuration loaded.'
})
